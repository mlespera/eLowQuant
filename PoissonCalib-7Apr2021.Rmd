---
title: "All gBlock data"
author: "copyright 2021 Mary Lesperance"
date: '`r Sys.Date()`'
output:
  html_document:
    number_sections: yes
...
#ML See EcoF-PoissonBlank-gBlockAll-17June2020.Rmd
...
This file recreates the analyses in the publication, 
...
DNA samples' by Lesperance, Allison, Bergman, Hocking, Helbing, *eDNA*, 2021.

* Create a folder called Outputs in your working directory.
* Put files in working directory: PoissonCalib-Functions-7April2021.R, GEDWG_LOD_DATA3.csv (Klymus data), AssaySummary-ML.csv (Klymus results)
...
* The script uses observations with nonempty data and where phat <1 (num detect<num technical replicates)
...
* Adds in Negative Control (ntc) zeroes (24/(48 for Monroe) technical replicates).
...
source("PoissonCalib-Functions-7April2021.R")
...
DAT <- read.csv("GEDWG_LOD_DATA3.csv")  #gBlock data
dim(DAT)
...
using digital PCR on a nanofluidic device", POlS One, Vol 3, Issue 8, e2876,
...
# if(sink.indicator){
#   sink(file='gBlock-SummariesRaw.txt', split=TRUE)}
# for(i in uLabs){
...
dim(DAT.Tar.SQ); cat('dim, Added 24/48 negative controls for Targets with zero ntc') 
# head(DAT.Tar.SQ)
...
if(sink.indicator){
  sink(file='gBlock-Summaries.txt', split=TRUE)}
for(i in uLabs){
...
Blue line is least squares linear regression line.
...
## overlay simple regression line and R-squared
  ...
  abline(jlm, col=4)
  legend("topleft", paste('Rsq=',round(summary(jlm)$r.squared, 2)), bty="n")
  cat("\n\n")
}
...
This file contains the outputs from all 29 data sets from Klymus et al. (2019) whose data are summarized in Table 4 of the manuscript. Results from both no intercept and intercept models are presented.
...
```{r MLES0fits0, warning=FALSE, echo=FALSE, eval=FALSE, include=FALSE}  
  ...
  # Values of (nd0, nn0) such that nd0/nn0 < 1 - exp(-alpha_hat)
...
# Determine Lc, Ld, Lq - no intercept model
...
values xd and NN replicates. False negative rate Ld computation:  P(Y <= Lc | p_xd) <= betaLd,
and solve for xd.
...
# Determine Lc, Ld, Lq - intercept model
...
# Estimates, Lc, Ld, Lq for a given number of technical reps NN[NNi]

Choose the model (intercept versus no intercept) with the best LLR test fit.
...
Include MYPI-6t and eASMO9, eFISH1, eONKI4 in paper.  
Index into uTargets (5, 25, 27, 31)


```{r Manuscript, echo=FALSE, eval=TRUE, include=TRUE}
# Include MYPI-6t and eASMO9, eFISH1, eONKI4 in paper.  (5, 25, 27, 31)
ManuTargets <- c(5, 25, 27, 31)
Manusink <- FALSE  #writes to output files when set to TRUE
#Manusink <- TRUE

if(Manusink){ 
      sink(file(paste("Outputs\\gBlockLimits.txt", sep=""), 
            encoding="UTF-8"), split=TRUE)
}
xdxq.choice.PR <- xdxq.choice[nrowTarget>2, -1]
xdxq.choice.PR <- xdxq.choice.PR[order(row.names(xdxq.choice.PR)), ]
colnames(xdxq.choice.PR)[6:11] <- c('LODL','LOD','LODU','LOQL','LOQ','LOQU')
Klymus <- read.csv('AssaySummary-ML.csv')
#xdxq.choice.PR <- cbind(Assay=Klymus[,1], xdxq.choice.PR, 

xdxq.choice.PR <- cbind(xdxq.choice.PR, 
                        K.LOD8=Klymus[,c(12)], K.LOQ=Klymus[,c(7)])
#write.csv(xdxq.choice.PR, 'Table3.csv')
...
if(Manusink){
  sink(file("Outputs\\gBlockData.txt", encoding="UTF-8"),
       split=TRUE)
  }
...
if(Manusink){ sink()}
 ...
 if(Manusink){
    sink(file(paste("Outputs\\gBlockResults.txt", sep=""), 
            encoding="UTF-8"), split=TRUE)
  }
...
#      ylim=c(0, maxlamhat), xlim=c(0, maxSQ), las=1, 
  ...
  plot(sqs, Calib.phat, xlog=TRUE, 
       ylab='Proportion detect', xlab='Starting copy number', type='l', col=4, las=1, 
       ylim=c(0, 1), xlim=c(0, maxSQa),  main=uTargets[Targeti])
  points(Target.dat$SQ, Target.dat$phat)
  legend("topleft", legend=c('ML fit'), lty=1, col=4, bty="n")
...
```{r Manuscript2, echo=FALSE, eval=TRUE, include=TRUE}
#Power plot for ManuTargets[1]
#pdf("Outputs\\PowerPlot.pdf")
par(mfrow=c(1,1))

SQs <- seq(.01, 4, by=.1)
PowerSQs <- matrix(0, nrow=length(SQs), ncol=length(NN))
colnames(PowerSQs) <- NN
for (j in 1:length(NN)){
   PowerSQs[,j] <- pbinom(0, size=NN[j], prob=1-exp(-(alphabeta0.se[ManuTargets[1],3] * SQs)),
                          lower.tail = FALSE)
}

jBrewColors <- brewer.pal(n = 8, name = "Dark2")
matplot(SQs, PowerSQs, type='l', col=jBrewColors, lty=1:8, las=1, 
        xlab="Starting copy number", ylab="Power")
 #       main=paste(uTargets[ManuTargets[1]], 
  #                 'Power vs S for numbers of technical reps', sep=', '))
legend('bottomright', paste(NN, sep=''), lty=1:8, col=jBrewColors, bty='n',
