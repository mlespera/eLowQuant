...
#  nn0 <- 24   
  ...
  if(sink.indicator) sink(file(paste("Outputs\\MLNoInterSQ0",uTargets[i],
            ".txt", sep=""), 
            encoding="UTF-8"), split=TRUE)
  print(knitr::kable(cbind(NumDetects=1:nrow(SQS0.fit0), SQS0.fit0[,3:4]), 
...
#  nn0 <- 24   
  ...
  print(knitr::kable(cbind(NumDetects=1:nrow(SQS0.fit), SQS0.fit[,5:6]), 
                     format="pandoc", digits=3),results="asis")
...
# Intercept Limits table
{
if(Manusink){ 
      sink(file(paste("Outputs\\LimitsInter.txt", sep=""), 
            encoding="UTF-8"), split=TRUE)
}
...
# No Intercept Limits table
{
if(Manusink){ 
      sink(file(paste("Outputs\\LimitsNoInter.txt", sep=""), 
            encoding="UTF-8"), split=TRUE)
}
...
# Summary stats for all assays which were modelled
{
if(Manusink){
...
#V20220912 printed all model output results for both intercept and no-intercept models
{
 if(Manusink){
...
#V20220912 plotted all output results for both intercept and no-intercept models
...
#V20220912 plotted all limit results for both intercept and no-intercept models
